(() => {
  ...
  function fillGapsFromDataset() {
    ...
    if (!ap || !db || typeof db !== 'object') {
      return { added: 0, manual: 0, skipped: true };
    }
    let added = 0;
    ...
    return { added: added, manual: manual, skipped: false };
  }

  const first = fillGapsFromDataset();
  window.__STX_PATCH_V11 = first.skipped
    ? { note: 'deferred: STX_DATASET.ALL_PARTS or PARTS_DB missing at load; will retry on dataset ready' }
    : { note: 'filled PARTS_DB from STX_DATASET.ALL_PARTS for missing idRaw keys', addedKeys: first.added, manualGapKeys: first.manual };

  const prevReady = window.__ccOnStxDatasetReady;
  window.__ccOnStxDatasetReady = function () {
    try {
      if (typeof prevReady === 'function') prevReady();
    } catch (_) {}
    const again = fillGapsFromDataset();
    window.__STX_PATCH_V11 = {
      note: 'filled PARTS_DB from STX_DATASET.ALL_PARTS for missing idRaw keys (post-ready)',
      addedKeys: again.added,
      manualGapKeys: again.manual
    };
  };
})();
